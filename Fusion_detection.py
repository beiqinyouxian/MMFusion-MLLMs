...
def generate_specific_attention_map(model, processor, image, object, label, attention_type, model_type, image_type):
    ...
    # Qwen待修改
        ...
        vi_items=["9_6","1_1","8_4","4_2","2_6","11_6","10_6","9_2","7_8","10_2"][:numofheads]
        ir_items=["9_6","4_2","2_6","8_4","11_6","10_6","7_7","11_3","4_7","7_8"][:numofheads]
...
def get_fusion_attention_map(model, processor, ir_img, vi_img, label, object, attention_type, model_type):
    ...
    model_att_results["vi"] = generate_specific_attention_map(model, processor, ir_img, object, label, attention_type, model_type,image_type="vi")
    model_att_results["ir"] = generate_specific_attention_map(model, processor, vi_img, object, label, attention_type, model_type, image_type="ir")
    ...
    eval_processor = Evaluation_processor(label, threshold_attention_map)
    # 计算评估指标
...
if __name__ == "__main__":
    ...
    # for num save   
    model_name = (args.model_path).split('/')[-1]     
    with open(f"./num_results/{model_name}_{args.num}_{dataset_processor.dataset_type}_output_{args.dataset}_{args.eval_type}_{args.attention_type}.txt", "a") as file:
